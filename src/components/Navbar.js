// Navbar.jsx
"use client";

import { useState, useMemo } from "react";
import Link from "next/link";
import { usePathname, useRouter } from "next/navigation";
import AuthModal from "./AuthModal";
import { useAuth } from "@/context/AuthContext";

// Component to handle navigation override
const NavItem = ({ href, label, pathname, onClick, className }) => {
    const router = useRouter();

    const handleClick = (e) => {
        e.preventDefault(); 
        if (onClick) onClick();
        router.push(href);
    };
    
    return (
        <span
            onClick={handleClick}
            className={`${className} cursor-pointer`}
        >
            {label}
        </span>
    );
};


export default function Navbar() {
    const [isOpen, setIsOpen] = useState(false);
    const [isOpenForm, setIsOpenForm] = useState(false);
    const [authMode, setAuthMode] = useState("login");
    const pathname = usePathname();
    const router = useRouter(); 

    const { user, logout } = useAuth();
    
    // Define all links (assuming role logic from previous steps)
    const allLinks = [
        { href: "/", label: "Home", roles: ["guest", "user", "admin"] },
        { href: "/search", label: "Search", roles: ["guest", "user", "admin"] },
        { href: "/about", label: "About Us", roles: ["guest", "user", "admin"] },
        { href: "/faqs", label: "FAQs", roles: ["guest", "user", "admin"] },
        { href: "/contact", label: "Contact", roles: ["guest", "user", "admin"] },
        { href: "/dashboard", label: "Dashboard", roles: ["user", "admin"] },
        { href: "/admin", label: "Admin", roles: ["admin"] },
    ];

    const navLinks = useMemo(() => {
        const userRole = user ? user.role : 'guest';
        return allLinks.filter(link => 
            link.roles.includes(userRole)
        );
    }, [user]); 

    const linkBaseClass = "transition-colors duration-200";

    return (
        <>
            <nav className="bg-[#0a1128] text-white sticky top-0 z-50">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center justify-between h-16">
                        {/* Logo */}
                        <div className="flex-shrink-0">
                            <Link href="/">
                                <img src="/logo.png" alt="Logo" className="h-10" />
                            </Link>
                        </div>

                        {/* Desktop Menu (Desktop content remains unchanged) */}
                        <div className="hidden md:flex space-x-6">
                            {navLinks.map((link) => (
                                <NavItem
                                    key={link.href}
                                    href={link.href}
                                    label={link.label}
                                    pathname={pathname}
                                    className={`${linkBaseClass} ${
                                        pathname === link.href
                                            ? "text-blue-800 font-semibold"
                                            : "hover:text-blue-600"
                                    }`}
                                />
                            ))}
                        </div>

                        {/* Auth buttons / User Info (Desktop) */}
                        <div className="hidden md:flex space-x-4">
                            {user ? (
                                <>
                                    <span className="px-4 py-2 rounded-md bg-gray-700">
                                        {user.name ||user.Name || user.email}
                                    </span>
                                    <button
                                        onClick={logout}
                                        className="px-4 py-2 rounded-md bg-red-600 hover:bg-red-500"
                                    >
                                        Logout
                                    </button>
                                </>
                            ) : (
                                <>
                                    <button
                                        onClick={() => {
                                            setAuthMode("login");
                                            setIsOpenForm(true);
                                        }}
                                        id="login"
                                        className="px-4 py-2 rounded-md hover:bg-gray-700"
                                    >
                                        Sign In
                                    </button>
                                    <button
                                        onClick={() => {
                                            setAuthMode("signup");
                                            setIsOpenForm(true);
                                        }}
                                        className="px-4 py-2 rounded-md bg-blue-800 hover:bg-blue-700"
                                    >
                                        Sign Up
                                    </button>
                                </>
                            )}
                        </div>

                        {/* Mobile Button - SVG and Conditional Logic */}
                        <div className="md:hidden">
                            <button
                                onClick={() => setIsOpen(!isOpen)}
                                aria-label="Toggle mobile menu"
                                className="p-2 rounded-md text-gray-400 hover:text-white focus:outline-none focus:ring-2 focus:ring-white"
                            >
                                {isOpen ? (
                                    // Close (X) Icon
                                    <svg
                                        className="w-6 h-6"
                                        fill="none"
                                        stroke="currentColor"
                                        viewBox="0 0 24 24"
                                    >
                                        <path
                                            strokeLinecap="round"
                                            strokeLinejoin="round"
                                            strokeWidth={2}
                                            d="M6 18L18 6M6 6l12 12"
                                        />
                                    </svg>
                                ) : (
                                    // Hamburger Icon
                                    <svg
                                        className="w-6 h-6"
                                        fill="none"
                                        stroke="currentColor"
                                        viewBox="0 0 24 24"
                                    >
                                        <path
                                            strokeLinecap="round"
                                            strokeLinejoin="round"
                                            strokeWidth={2}
                                            d="M4 6h16M4 12h16M4 18h16"
                                        />
                                    </svg>
                                )}
                            </button>
                        </div>
                    </div>
                </div>

                {/* Mobile Menu - TRANSITIONS APPLIED HERE */}
                <div 
                    className={`md:hidden transform transition-all duration-300 ease-in-out overflow-hidden ${
                        isOpen 
                            ? 'max-h-screen opacity-100 p-4' 
                            : 'max-h-0 opacity-0 p-0'
                    }`}
                >
                    {isOpen && (
                        <div className="space-y-2">
                            {navLinks.map((link) => (
                                <NavItem
                                    key={link.href}
                                    href={link.href}
                                    label={link.label}
                                    pathname={pathname}
                                    className={`block px-4 py-2 rounded-md ${linkBaseClass} ${
                                        pathname === link.href
                                            ? "bg-blue-600 text-white"
                                            : "hover:bg-gray-700"
                                    }`}
                                    onClick={() => {
                                        setIsOpen(false);
                                        // The NavItem handles router.push
                                    }}
                                />
                            ))}
                            
                            {user ? (
                                <>
                                    <span className="block px-4 py-2 rounded-md bg-gray-700">
                                        {user.name || user.email}
                                    </span>
                                    <button
                                        onClick={() => {
                                            logout();
                                            setIsOpen(false);
                                        }}
                                        className="block w-full text-left px-4 py-2 rounded-md bg-red-600 hover:bg-red-500"
                                    >
                                        Logout
                                    </button>
                                </>
                            ) : (
                                <>
                                    <button
                                        onClick={() => {
                                            setAuthMode("login");
                                            setIsOpenForm(true);
                                            setIsOpen(false);
                                        }}
                                        className="block w-full text-left px-4 py-2 rounded-md hover:bg-gray-700"
                                        id="login"
                                    >
                                        Sign In
                                    </button>
                                    <button
                                        onClick={() => {
                                            setAuthMode("signup");
                                            setIsOpenForm(true);
                                            setIsOpen(false);
                                        }}
                                        className="block w-full text-left px-4 py-2 rounded-md bg-blue-800 hover:bg-blue-700"
                                    >
                                        Sign Up
                                    </button>
                                </>
                            )}
                        </div>
                    )}
                </div>
            </nav>

            {isOpenForm && (
                <AuthModal mode={authMode} onClose={() => setIsOpenForm(false)} />
            )}
        </>
    );
}